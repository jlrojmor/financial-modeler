...
export default function SbcAnnotation() {
  ...
  // Calculate total SBC for each year (SG&A + COGS)
  const totalSbcByYear = useMemo(() => {
    const totals: Record<string, number> = {};
    years.forEach((y) => {
      totals[y] = (sgaTotalSbcByYear[y] ?? 0) + (cogsTotalSbcByYear[y] ?? 0);
    });
    return totals;
  }, [sgaTotalSbcByYear, cogsTotalSbcByYear, years]);
  ...
  return (
    <div className="mt-8">
      <CollapsibleSection
        ...
        description="Document how much Stock-Based Compensation is embedded within your SG&A and COGS components. This is for transparency purposes only and does not affect Income Statement calculations."
        colorClass="amber"
        ...
        <div className="mt-4 rounded-lg border border-amber-700/30 bg-amber-950/10 p-3">
          <div className="text-[10px] text-amber-300/70 leading-relaxed">
            ...
            <p className="mb-1">
              ...
              (primarily within Selling, General & Administrative expenses and, if applicable, Cost of Goods Sold). 
